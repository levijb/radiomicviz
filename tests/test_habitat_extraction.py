"""
mri-voxelwise preset extraction tests.

mri-voxelwise is a curated first-order + GLCM + GLRLM subset designed for
...
Because the bin settings differ, we do not assert voxelwise feature names are a
...
class TestHabitatConfig:
    ...
    def test_preset_loads(self):
        """load_preset('mri-voxelwise') returns a dict with featureClass."""
        from radiomicviz.config import load_preset
        cfg = load_preset("mri-voxelwise")
        assert isinstance(cfg, dict)
        assert "featureClass" in cfg, (
            "mri-voxelwise YAML is missing 'featureClass' — preset would extract "
            "all feature classes instead of the intended curated subset."
        )

    def test_preset_has_image_type(self):
        """mri-voxelwise specifies at least one imageType."""
        from radiomicviz.config import load_preset
        cfg = load_preset("mri-voxelwise")
    ...
    def test_preset_has_bin_count(self):
        """mri-voxelwise setting section has binCount for intensity discretization."""
        from radiomicviz.config import load_preset
        cfg = load_preset("mri-voxelwise")
...
class TestHabitatEndToEnd:
    ...
    def test_extraction_completes(self, result):
        """mri-voxelwise extraction returns an ExtractionResult without error."""
    ...
    def test_has_features(self, result):
        """mri-voxelwise produces exactly 42 features: firstorder(18) + glcm(8) + glrlm(16)."""
        assert result.n_features == 42, (
            f"Expected 42 features (mri-voxelwise, PyRadiomics 3.0.1), got {result.n_features}"
        )
    ...
    def test_no_nan_values(self, result):
        ...
        if nan_features:
            ...
            pytest.fail(
                f"NaN in {len(nan_features)} voxelwise features: {nan_vals}. "
    ...
    def test_mostly_nonzero(self, result):
        ...
        pct = nonzero / total
        assert pct >= 0.80, f"Only {pct:.1%} of voxelwise features are non-zero"
...
class TestHabitatVsDefault:
    ...
    Would catch: mri-voxelwise.yaml missing featureClass (extracts everything),
    ...
    def results(self, synthetic_image, synthetic_mask_single):
        ...
        r_hab = extract(synthetic_image, synthetic_mask_single, preset="mri-voxelwise")
        return r_def, r_hab

    def test_curated_count(self, results):
        """mri-voxelwise produces fewer features than mri-default."""
        r_def, r_hab = results
        assert r_hab.n_features < r_def.n_features, (
            f"mri-voxelwise ({r_hab.n_features}) should have fewer features than "
            f"mri-default ({r_def.n_features}). "
            "Check that mri-voxelwise.yaml has a featureClass restriction."
        )

    def test_habitat_has_no_shape_features(self, results):
        """mri-voxelwise omits shape features (not meaningful in clustering / voxelwise)."""
        _, r_hab = results
...
class TestHabitatMultiLabel:
    """
    mri-voxelwise on multi-label mask → features for all labels.
    ...
    def test_all_labels_extracted(self, synthetic_image, synthetic_mask_multi):
        """mri-voxelwise on a 3-label mask returns features for labels 1, 2, 3."""
        from radiomicviz import extract
        result = extract(
            synthetic_image,
...
class TestHabitatCsvRoundTrip:
    ...
    def test_roundtrip(self, synthetic_image, habitat_mask, tmp_path):
        """Export voxelwise features to CSV and reload — column count matches."""
        from radiomicviz import extract
        result = extract(
            synthetic_image,
            habitat_mask,
...
class TestHabitatClusteringReady:
    """
    voxelwise features are finite floats ready for sklearn clustering.
    ...
    def test_all_finite_values(self, result):
        ...
        assert np.isfinite(df.values).all(), "Non-finite values in voxelwise features"

    def test_sklearn_scaler_succeeds(self, result):
        """StandardScaler.fit_transform succeeds on voxelwise features (skips if sklearn absent)."""
