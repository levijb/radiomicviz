...
# -------------------------------------------------------------------------
@cli.command()
@click.option("-i", "--image", default=None, type=click.Path(exists=True),
              help="Background image NIfTI. Auto-detected from --subject-dir if omitted.")
@click.option("--subject-dir", default=None, type=click.Path(exists=True, file_okay=False),
              help="Subject root directory. Subdirectories containing .nrrd files become regions; "
                   ".nii.gz files in the root become background image options.")
...
@click.option("-v", "--verbose", is_flag=True)
def view(image, subject_dir, mask, overlays, feature_4d, port, no_browser, verbose):
    """Launch interactive browser viewer for NIfTI / NRRD files."""
    _setup_logging(verbose)

    if image is None and subject_dir is None:
        raise click.UsageError("Provide --image, --subject-dir, or both.")
